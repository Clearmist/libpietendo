...
struct AccessControlInfo
{
    ...
    // the reason for the reverse ordering is because the normal serialization of this field is: "field |= save_id",
    // then "field = field << 20"
    ...
    static const size_t kMaxSystemCallValue =
        0x7d; // this is the maximum value for a system call according to the system
    ...
    static_assert(
        sizeof(ReleaseKernelVersionDescriptor) == sizeof(uint32_t),
        "ReleaseKernelVersionDescriptor had incorrect size.");
    ...
    static_assert(
        sizeof(HandleTableSizeDescriptor) == sizeof(uint32_t), "HandleTableSizeDescriptor had incorrect size.");
    ...
    static_assert(
        sizeof(OtherCapabilitiesDescriptor) == sizeof(uint32_t), "OtherCapabilitiesDescriptor had incorrect size.");

    // MappingStaticDescriptor come in pairs: [begin_page,end_page) where begin.flag == (true: range read-only, false:
    // range is read-write), and end.flag == (true: static mapping, false: io mapping)
  ...
  public:
    ...
    std::array<tc::bn::le16<uint16_t>, 16>
        resource_limit_descriptor; // for each descriptor use, see ResourceLimitDescriptorIndex
    union
    {
        tc::bn::le64<uint64_t> ext_savedata_id; // this field is used when
                                                // other_attributes.test(USE_EXTENDED_SAVEDATA_ACCESS_CONTROL) == false
        tc::bn::le64<AccessibleUniqueIds>
            accessible_unique_ids_1; // this field is used when
                                     // other_attributes.test(USE_EXTENDED_SAVEDATA_ACCESS_CONTROL) == true
    };
    std::array<tc::bn::le32<uint32_t>, 2> system_savedata_id;
    tc::bn::le64<AccessibleUniqueIds>
        accessible_unique_ids_0;          // other_attributes.test(USE_EXTENDED_SAVEDATA_ACCESS_CONTROL):
                                          // true = accessible_save_ids, false: other_user_save_ids
    tc::bn::bitarray<7> fs_access;        // FileSystemAccess
    tc::bn::bitarray<1> other_attributes; // OtherAttribute
    std::array<ServiceName, 34>
        service_access_control; // In firmware versions prior to 9.2.0(?) this array is only 32 elements long
    tc::bn::pad<0xf> padding0;
    byte_t resource_limit_category; // ResourceLimitCategory
    // end ARM11 System

    // begin ARM11 Kernel
    std::array<tc::bn::le32<uint32_t>, 28>
        kernel_descriptors; // Descripters are a collection of u32s, with masks to differentiate between different
                            // descs. Impl'd Order: SysCallControl, InteruptNumList, AddressMapping(ExHdr order: IoMaps,
                            // MemMaps, AccDesc order: MemMaps, IoMaps), OtherCapabilities, HandleTableSize,
