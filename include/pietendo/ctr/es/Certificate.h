 ...
 * @param[in] data Raw certificate data.
 *
 * @return Void pointer to certificate public. See @ref pie::es::ESCertRsa4096PublicKey @ref
 * pie::es::ESCertRsa2048PublicKey @ref pie::es::ESCertEcc233PublicKey.
 */
...
struct Certificate
{
  public:
    Certificate()
        : signature(), subject(), date(), public_key_type(), rsa4096_public_key(), rsa2048_public_key(),
          ecc233_public_key()
    {
  ...
  public:
    ...
    std::array<byte_t, 32>
        calculated_hash; // This hash is calculated when deserialised so that signature validation can be performed.
