...
struct CroHeader
{
    ...
    enum CoreFunctionIndex
    {
        ...
        CoreFunctionIndex_OnUnresolved =
            3, // will be called when an unresolved function is called. Set to 0xFFFFFFFF if not exists.
    };
    ...
    tc::bn::le32<uint32_t>
        node0; // Next loaded CRO pointer, set by RO during loading (Usually zero when the CRO is being loaded)
...
struct CroNamedImportTableEntry
{
    tc::bn::le32<uint32_t> name_offset;
    tc::bn::le32<uint32_t>
        import_patch_list_offset; // Offset of the head of a linear list that contains the patches for this import
};
...
struct CroImportModuleTableEntry
{
    ...
    tc::bn::le32<uint32_t>
        anonynous_import_patch_list_offset; // Offset of the head of a sub list in Anonymous Import Table
};
...
struct CroPatchEntry
{
    enum PatchType : byte_t
    {
        ...
        PatchType_WriteU32Relative_2 = 42, // (((signed int)base*2)/2+addend-in_ptr), otherwise err) (This is apparently
                                           // a subset of relocation type for ARM ELF)
    };

    tc::bn::le32<CroSegmentOffset> segment_offset_for_output;
    PatchType patch_type;
    byte_t
        unk0; // For import patches, non-zero if last entry; for relocation patches, this is the referred segment index
    byte_t unk1; // For import patches, 1 is written to first entry if all symbols loaded successfully; unknown
                 // (padding?) for relocation patches
