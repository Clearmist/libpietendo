...
struct NcchCommonHeader
{
     ...
     * @details
     * The current version is 2. However due to a bug in the SDK, CFAs do not have this field set, making this field
     * unreliable.
     */
    enum FormatVersion
    {
    ...
    enum FlagIndex
    {
        ...
        FlagIndex_BlockSizeLog = 6, /**< NCCH BlockSize (`block_size = 1 << (block_size_log + 9)`, alternatively this is
                                       always 0, and the blocksize is always 0x200) */
    ...
    enum OtherFlag
    {
        ...
        OtherFlag_ManualDisclosure =
            6, /**< Manual does not require KeyY seed (only required if application does required seed) */
    };
    ...
    struct NcchFlags
    {
        ...
        byte_t security_version; // this determines the secure crypto mode, where != 0 this uses a different key for:
                                 // romfs, & non .icon&.banner exefs
        tc::bn::bitarray<1> content_platform;
        ContentTypeFlag content_flag;
        byte_t block_size_log; // this flag determines the block_size = 1 << (block_size_log + 9), alternatively this is
                               // always 0, and the blocksize is always 0x200
        ...
        {
            return ((const byte_t *)this)[index];
        }
    ...
    // 0x80
    tc::bn::le32<uint32_t>
        exhdr_size; // note that this size does not include the access_desc binary that follows the exheader
