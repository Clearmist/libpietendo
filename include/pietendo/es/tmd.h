...
enum ESContentType : uint16_t
{
    ...
    ESContentType_HASHED =
        (1 << 1), /**< Hashed (Previously this was called DISC, because only RVL discs had the hashed data) - (bit1) */
...
struct ESTitleMetaHeader
{
    ...
    uint8_t platformVersion;  /**< Platform version number. Used only in WiiU to differentiate between Wii and vWii TMDs
                                 (set to 1 for vWii). */
