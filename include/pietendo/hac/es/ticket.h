...
struct sContentRecord_v1
{
  ...
  public:
    uint32_t index_group() const
    {
        return group.unwrap();
    }
    bool is_index_enabled(uint16_t index) const
    {
        return access_mask.test(get_access_mask(index));
    }
    /*
    {
    ...
    */
    ...
    void clear()
    {
        memset(this, 0, sizeof(sContentRecord_v1));
    }

    void set_index_group(uint16_t index)
    {
        group.wrap(get_group(index));
    }
    void enable_index(uint16_t index)
    {
        access_mask.set(get_access_mask(index));
    } //{ access_mask[get_access_mask(index) / 8] |= BIT(get_access_mask(index) % 8); }
    void disable_index(uint16_t index)
    {
        access_mask.reset(get_access_mask(index));
    } //{ access_mask[get_access_mask(index) / 8] &= ~BIT(get_access_mask(index) % 8); }

    inline uint16_t get_access_mask(uint16_t index) const
    {
        return index & kAccessMaskMask;
    }
    inline uint16_t get_group(uint16_t index) const
    {
        return index & kGroupMask;
    }
};
#pragma pack(pop)
