...
struct RomFsSnapshotGenerator : public tc::io::VirtualFileSystem::FileSystemSnapshot
{
  ...
  private:
    ...
    inline pie::hac::sRomfsDirEntry *getDirEntry(uint32_t vaddr)
    {
        return (pie::hac::sRomfsDirEntry *)(mDirEntryTable.data() + vaddr);
    }

    tc::ByteData mFileEntryTable;
    inline pie::hac::sRomfsFileEntry *getFileEntry(uint32_t vaddr)
    {
        return (pie::hac::sRomfsFileEntry *)(mFileEntryTable.data() + vaddr);
    }
