...
class HierarchicalSha256Stream : public tc::io::IStream
{
  public:
     ...
     *the number of bytes written. @ref write is not supported for @ref HierarchicalSha256Stream.
  ...
  private:
    ...
    inline size_t offsetToBlock(int64_t offset)
    {
        return tc::io::IOUtil::castInt64ToSize(offset / tc::io::IOUtil::castSizeToInt64(mDataStreamBlockSize));
    };
    inline size_t offsetInBlock(int64_t offset)
    {
        return tc::io::IOUtil::castInt64ToSize(offset % tc::io::IOUtil::castSizeToInt64(mDataStreamBlockSize));
    };
    inline int64_t blockToOffset(size_t block)
    {
        return tc::io::IOUtil::castSizeToInt64(block) * tc::io::IOUtil::castSizeToInt64(mDataStreamBlockSize);
    };
    inline size_t getSizeOfBlock(size_t block)
    {
        return block < mDataStreamFinalBlockIndex ? mDataStreamBlockSize : mDataStreamFinalBlockSize;
    }

    // hash cache for data layer
    tc::ByteData mHashCache;
    inline byte_t *getBlockHash(size_t begin_block)
    {
        return mHashCache.data() + (begin_block * tc::crypto::Sha2256Generator::kHashSize);
    }
    ...
    bool validateLayerBlocksWithHashLayer(
