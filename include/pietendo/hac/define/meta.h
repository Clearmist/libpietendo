...
struct sMetaHeader
{
    ...
    struct sFlag
    {
        ...
        byte_t optimise_memory_allocation : 1; // if KernelCaps.MiscParams.ProgramType == Application &&
                                               // system_resource_size != 0 this will be set if
                                               // MemoryAllocationCompatiblityMode is false
        byte_t : 0;
    } flag;
    byte_t reserved_1;
    ...
    tc::bn::le32<uint32_t>
        system_resource_size; // this can only be set if KernelCaps.MiscParams.ProgramType is Application or Applet and
                              // flag.process_address_space == AddressSpace64Bit. Using this makes SystemCalls
                              // MapPhysicalMemory & UnmapPhysicalMemory unavailable.
