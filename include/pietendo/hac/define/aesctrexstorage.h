...
{
    union
    { // this union ensure size of kNodeSize
        struct
        {
            struct
            {
            ...
            std::array<
                sAesCtrExStorageEntry,
                (aesctrexstorage::kNodeSize - sizeof(header)) / sizeof(sAesCtrExStorageEntry)>
                entries;
        };
...
struct sAesCtrExStorageBlock
{
    ...
    std::array<tc::bn::le64<uint64_t>, (0x4000 - sizeof(header)) / sizeof(tc::bn::le64<uint64_t>)>
        base_physical_bucket_offset;
