...
class KernelCapabilityEntry
{
  ...
  private:
    ...
    inline uint32_t getFieldShift() const
    {
        return (uint32_t)mType + 1;
    }
    inline uint32_t getFieldMask() const
    {
        return (1 << (31 - (uint32_t)mType)) - 1;
    }
    inline uint32_t getCapMask() const
    {
        return (1 << ((uint32_t)mType)) - 1;
    }
