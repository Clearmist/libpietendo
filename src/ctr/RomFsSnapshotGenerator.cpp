...
pie::ctr::RomFsSnapshotGenerator::RomFsSnapshotGenerator(const std::shared_ptr<tc::io::IStream> &stream)
    : FileSystemSnapshot(), mBaseStream(stream), mDataOffset(0), mDirEntryTable(), mDirParentVaddrMap(),
      mFileEntryTable()
{
    ...
    if (mBaseStream->canRead() == false || mBaseStream->canSeek() == false)
    {
        throw tc::NotSupportedException(
            "pie::ctr::RomFsSnapshotGenerator", "Input stream requires read/seek permissions.");
    }
    ...
    std::cout << "expected data offset        : " << align<uint32_t>(hdr.file_entry.offset.unwrap() +
    hdr.file_entry.size.unwrap(), pie::ctr::RomFsHeader::kRomFsDataAlignSize) << std::endl;
    */

    if (hdr.header_size.unwrap() != sizeof(pie::ctr::RomFsHeader) ||
        hdr.dir_hash_bucket.offset.unwrap() != sizeof(pie::ctr::RomFsHeader) ||
        hdr.data_offset.unwrap() != align<uint32_t>(
                                        hdr.file_entry.offset.unwrap() + hdr.file_entry.size.unwrap(),
                                        pie::ctr::RomFsHeader::kRomFsDataAlignSize))
    {
    ...
    for (uint32_t v_addr = 0; v_addr < mDirEntryTable.size();)
    {
            ...
            std::cout << " > hash_sibling: 0x" << std::hex << getDirEntry(v_addr)->hash_sibling_offset.unwrap() <<
    std::endl; std::cout << " > name_size:    0x" << std::hex << getDirEntry(v_addr)->name_size.unwrap() << std::endl;

            uint32_t total_size = sizeof(pie::ctr::RomFsDirectoryEntry) +
    align<uint32_t>(getDirEntry(v_addr)->name_size.unwrap(), 4); std::cout << " > entry_size:   0x" << std::hex <<
    total_size << std::endl;
    ...
    */
    ...
    for (uint32_t v_addr = 0; v_addr < mFileEntryTable.size();)
    {
            ...
            std::cout << " > sibling:      0x" << std::hex << getFileEntry(v_addr)->sibling_offset.unwrap() <<
    std::endl; std::cout << " > data_offset:  0x" << std::hex << getFileEntry(v_addr)->data_offset.unwrap() <<
    std::endl; std::cout << " > data_size:    0x" << std::hex << getFileEntry(v_addr)->data_size.unwrap() << std::endl;
            std::cout << " > hash_sibling: 0x" << std::hex << getFileEntry(v_addr)->hash_sibling_offset.unwrap() <<
    std::endl; std::cout << " > name_size:    0x" << std::hex << getFileEntry(v_addr)->name_size.unwrap() << std::endl;

            uint32_t total_size = sizeof(pie::ctr::RomFsFileEntry) +
    align<uint32_t>(getFileEntry(v_addr)->name_size.unwrap(), 4); std::cout << " > entry_size:   0x" << std::hex <<
    total_size << std::endl;
    ...
    */
    ...
    if (mDirEntryTable.size() == 0 || getDirEntry(0)->parent_offset.unwrap() != 0 ||
        getDirEntry(0)->sibling_offset.unwrap() != 0xffffffff || getDirEntry(0)->name_size.unwrap() != 0)
    {
        throw tc::ArgumentOutOfRangeException(
            "pie::ctr::RomFsSnapshotGenerator", "Root RomFsDirectoryEntry corrupted.");
    }
    ...
    for (uint32_t v_addr = 0; v_addr < mDirEntryTable.size();)
    {
        ...
        uint32_t entry_total_size =
            sizeof(pie::ctr::RomFsDirectoryEntry) + align<uint32_t>(getDirEntry(v_addr)->name_size.unwrap(), 4);

        if (getDirEntry(v_addr)->sibling_offset.unwrap() < v_addr)
        {
            throw tc::InvalidOperationException(
                "pie::ctr::RomFsSnapshotGenerator", "Possibly corrupted directory entry");
        }
    ...
    for (uint32_t v_addr = 0; v_addr < mFileEntryTable.size();)
    {
        ...
        if (getFileEntry(v_addr)->data_size.unwrap() != 0)
        {
            ...
            file_tmp.stream = std::shared_ptr<tc::io::SubStream>(new tc::io::SubStream(
                mBaseStream, mDataOffset + getFileEntry(v_addr)->data_offset.unwrap(),
        ...
        uint32_t total_size =
            sizeof(pie::ctr::RomFsFileEntry) + align<uint32_t>(getFileEntry(v_addr)->name_size.unwrap(), 4);
...
void pie::ctr::RomFsSnapshotGenerator::addFile(const pie::ctr::RomFsFileEntry *file_entry, size_t parent_dir)
{
    ...
    if (file_entry->data_size.unwrap() != 0)
    {
        ...
        tmp.stream = std::shared_ptr<tc::io::SubStream>(new tc::io::SubStream(
            mBaseStream, mDataOffset + file_entry->data_offset.unwrap(), file_entry->data_size.unwrap()));
        // std::cout << "file was added" << std::endl;
    }
    else
    {
...
void pie::ctr::RomFsSnapshotGenerator::addDirectory(const pie::ctr::RomFsDirectoryEntry *dir_entry, size_t parent_dir)
{
    ...
    if (dir_entry->parent_offset.unwrap() == 0 && dir_entry->sibling_offset.unwrap() == 0xffffffff &&
        dir_entry->name_size.unwrap() == 0)
    {
        // create dir path
        tc::io::Path dir_path = tc::io::Path("/");
    ...
    // add file children
    for (uint32_t child = dir_entry->file_offset.unwrap(); child != 0xffffffff;
    ...
    // add dir children
    for (uint32_t child = dir_entry->child_offset.unwrap(); child != 0xffffffff;
