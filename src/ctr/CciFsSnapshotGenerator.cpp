...
pie::ctr::CciFsShapshotGenerator::CciFsShapshotGenerator(const std::shared_ptr<tc::io::IStream> &stream)
    : FileSystemSnapshot(), mBaseStream(stream), mCurDir(0)
{
    ...
    if (mBaseStream->canRead() == false || mBaseStream->canSeek() == false)
    {
        throw tc::NotSupportedException(
            "pie::ctr::CciFsShapshotGenerator", "Input stream requires read/seek permissions.");
    }
    ...
    if (hdr.ncsd_header.struct_magic.unwrap() != hdr.ncsd_header.kStructMagic)
    {
        throw tc::ArgumentOutOfRangeException(
            "pie::ctr::CciFsShapshotGenerator", "CCI header had invalid struct magic.");
    }
    if (hdr.ncsd_header.flags.media_type != hdr.ncsd_header.MediaType_Card1 &&
        hdr.ncsd_header.flags.media_type != hdr.ncsd_header.MediaType_Card2)
    {
        throw tc::ArgumentOutOfRangeException(
            "pie::ctr::CciFsShapshotGenerator", "CCI header had unexpected media type.");
    }
    if (hdr.ncsd_header.flags.block_size_log != 0)
    {
        throw tc::ArgumentOutOfRangeException(
            "pie::ctr::CciFsShapshotGenerator", "CCI header had unexpected block size.");
    }
    if (hdr.ncsd_header.flags.media_platform.test(hdr.ncsd_header.MediaPlatform_CTR) == false)
    {
        throw tc::ArgumentOutOfRangeException(
            "pie::ctr::CciFsShapshotGenerator", "CCI header had unexpected supported platforms.");
    }
    ...
    for (size_t i = 0; i < partition.size(); i++)
    {
        ...
        {
            throw tc::ArgumentOutOfRangeException(
                "pie::ctr::CciFsShapshotGenerator", "CCI partition had unexpected fs type.");
        }
        if (hdr.ncsd_header.partition_crypto_type[i] != hdr.ncsd_header.PartitionCryptoType_None)
        {
            throw tc::ArgumentOutOfRangeException(
                "pie::ctr::CciFsShapshotGenerator", "CCI partition had unexpected crypto type.");
        }
    ...
    {
        throw tc::ArgumentOutOfRangeException(
            "pie::ctr::CciFsShapshotGenerator", "Input stream is too small, given calculated CCI partition geometry.");
    }
    ...
    // initial data
    // addFile("initial_data.bin", sizeof(pie::ctr::NcsdCommonHeader) + 0xe00,
    // sizeof(pie::ctr::CardInfoHeader::InitialData));
