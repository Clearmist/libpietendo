...
pie::ctr::es::RsaSigner::RsaSigner(
    pie::es::ESSigType sig_type, const std::string &issuer, const tc::crypto::RsaKey &rsa_key)
    : mSigType(sig_type), mIssuer(issuer), mRsaKey(rsa_key)
{
    switch (mSigType)
    {
    case pie::es::ESSigType_RSA4096_SHA1:
    case pie::es::ESSigType_RSA4096_SHA2256:
        if ((mRsaKey.n.size() << 3) != 4096)
            throw tc::ArgumentOutOfRangeException(
                "pie::ctr::es::RsaSigner::RsaSigner()",
                "Key size inferred from SigType did not match actual key size.");
        break;
    case pie::es::ESSigType_RSA2048_SHA1:
    case pie::es::ESSigType_RSA2048_SHA2256:
        if ((mRsaKey.n.size() << 3) != 2048)
            throw tc::ArgumentOutOfRangeException(
                "pie::ctr::es::RsaSigner::RsaSigner()",
                "Key size inferred from SigType did not match actual key size.");
        break;
    default:
        throw tc::ArgumentOutOfRangeException(
            "pie::ctr::es::RsaSigner::RsaSigner()", "SigType not supported for RsaSigner.");
        break;
    }
}
