...
pie::ctr::es::TicketDeserialiser::TicketDeserialiser(const std::shared_ptr<tc::io::IStream> &tik_stream)
    : Ticket(), mModuleLabel("pie::ctr::es::TicketDeserialiser")
{
    ...
    if (tik->v1Head.sectHdrEntrySize.unwrap() != sizeof(pie::es::ESV1SectionHeader))
    {
        throw tc::ArgumentOutOfRangeException(
            mModuleLabel, "TIK v1 header extension had unexpected size for section headers.");
    }
    ...
    fmt::print("titleKey:         {}\n", tc::cli::FormatUtil::formatBytesAsString(tik->head.titleKey.data(),
    tik->head.titleKey.size(), true, "")); fmt::print("ticketId:         {:016x}\n", tik->head.ticketId.unwrap());
    fmt::print("deviceId:         {:08x}\n", tik->head.deviceId.unwrap());
    fmt::print("titleId:          {:016x}\n", tik->head.titleId.unwrap());
    fmt::print("sysAccessMask:    {}\n", tc::cli::FormatUtil::formatBytesAsString(tik->head.sysAccessMask.data(),
    tik->head.sysAccessMask.size(), true, "")); fmt::print("ticketVersion:    {:d}\n",
    tik->head.ticketVersion.unwrap()); fmt::print("accessTitleId:    {:08x}\n", tik->head.accessTitleId.unwrap());
    ...
    fmt::print("customData:       {}\n", tc::cli::FormatUtil::formatBytesAsString(tik->head.customData.data(),
    tik->head.customData.size(), true, "")); fmt::print("audit:            {:02x}\n", tik->head.audit);
    fmt::print("cidxMask:         {}\n", tc::cli::FormatUtil::formatBytesAsString(tik->head.cidxMask.data(),
    tik->head.cidxMask.size(), true, "")); for (size_t i = 0; i < tik->head.limits.size(); i++)
    {
            fmt::print("lp entry {}:       code={:08x}, limit={:08x}\n", i, tik->head.limits[i].code.unwrap(),
    tik->head.limits[i].limit.unwrap());
    }
    ...
    //pie::es::ESV1SectionHeader* sect_hdr = (pie::es::ESV1SectionHeader*)(tik_data.data() + sizeof(pie::es::ESTicket) +
    tik->v1Head.sectHdrOfst.unwrap()); for (size_t i = 0; i < sect_hdr_num; i++)
    {
            fmt::print("sectHdr {:d}:\n", i);
        ...
        if (tik->sectHdrs[i].sectionType.unwrap() == (uint32_t)pie::es::ESItemType_CONTENT)
        {
            if (tik->sectHdrs[i].recordSize.unwrap() != sizeof(pie::es::ESV1ContentRecord))
            {
                throw tc::InvalidOperationException(
                    mModuleLabel, fmt::format(
                                      "Invalid size for ESV1ContentRecord. (expected: 0x{:x}, got 0x{:x})",
                                      sizeof(pie::es::ESV1ContentRecord), tik->sectHdrs[i].recordSize.unwrap()));
            }
            pie::es::ESV1ContentRecord *content_records =
                (pie::es::ESV1ContentRecord
                     *)(tik_data.data() + sizeof(pie::es::ESTicket) + tik->sectHdrs[i].sectOfst.unwrap());
