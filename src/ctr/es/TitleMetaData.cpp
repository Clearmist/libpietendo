...
pie::ctr::es::TitleMetaDataDeserialiser::TitleMetaDataDeserialiser(const std::shared_ptr<tc::io::IStream> &tmd_stream)
    : TitleMetaData(), mModuleLabel("pie::ctr::es::TitleMetaDataDeserialiser")
{
    ...
    */
    ...
    tc::crypto::GenerateSha2256Hash(
        calculated_hash.data(), (byte_t *)&tmd->sig.issuer,
        (size_t)((byte_t *)&tmd->v1Head.cmdGroups - (byte_t *)&tmd->sig.issuer));
    ...
    */
    ...
    for (size_t i = 0; i < cmd_table_num; i++)
    {
        this->content_info.push_back(ContentInfo(
            tmd->contents[i].cid.unwrap(), tmd->contents[i].index.unwrap(),
            (tmd->contents[i].type.unwrap() & (uint16_t)pie::es::ESContentType_ENCRYPTED) ==
                (uint16_t)pie::es::ESContentType_ENCRYPTED,
            (tmd->contents[i].type.unwrap() & (uint16_t)pie::es::ESContentType_OPTIONAL) ==
                (uint16_t)pie::es::ESContentType_OPTIONAL,
            (int64_t)tmd->contents[i].size.unwrap(), tmd->contents[i].hash));
    }
}