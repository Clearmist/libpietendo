...
pie::ctr::IvfcStream::IvfcStream()
    : mModuleLabel("pie::ctr::IvfcStream"), mBaseStream(), mDataStreamBlockSize(0), mDataStreamLogicalLength(0),
      mDataStream(), mHashCache(), mHash()
{
}

pie::ctr::IvfcStream::IvfcStream(const std::shared_ptr<tc::io::IStream> &stream) : IvfcStream()
{
    mBaseStream = stream;
    ...
    for (size_t i = 0; i < section.size(); i++)
    {
        if (i == MasterHash)
        {
            section[i].offset = int64_t(
                align<size_t>(sizeof(pie::ctr::IvfcCtrRomfsHeader), pie::ctr::IvfcCtrRomfsHeader::kHeaderAlign));
        ...
        else
        {
            ...
            section[i].block_num = size_t(section[i].size / int64_t(section[i].block_size)) +
                                   (size_t(section[i].size % int64_t(section[i].block_size)) ? 1 : 0);
    ...
    // set actual offsets
    section[DataLevel2].offset =
        align<int64_t>(section[MasterHash].offset + section[MasterHash].size, section[DataLevel2].block_size);
    section[HashLevel0].offset =
        align<int64_t>(section[DataLevel2].offset + section[DataLevel2].size, section[HashLevel0].block_size);
    section[HashLevel1].offset =
        align<int64_t>(section[HashLevel0].offset + section[HashLevel0].size, section[HashLevel1].block_size);
    ...
    auto hash_level0_data =
        tc::ByteData(align<size_t>(static_cast<size_t>(section[HashLevel0].size), section[HashLevel0].block_size));
    auto hash_level1_data =
        tc::ByteData(align<size_t>(static_cast<size_t>(section[HashLevel1].size), section[HashLevel1].block_size));
    ...
    */
    ...
    // validate level0
    if (validateLayerBlocksWithHashLayer(
            hash_level0_data.data(), section[HashLevel0].block_size, section[HashLevel0].block_num,
            master_hash_data.data()) == false)
    {
    ...
    // validate level1
    if (validateLayerBlocksWithHashLayer(
            hash_level1_data.data(), section[HashLevel1].block_size, section[HashLevel1].block_num,
            hash_level0_data.data()) == false)
    {
        throw tc::crypto::CryptoException("pie::ctr::IvfcStream", "Hash layer0 failed hash validation.");
    }
    ...
    mDataStream = std::shared_ptr<tc::io::SubStream>(new tc::io::SubStream(
        mBaseStream, section[DataLevel2].offset,
        tc::io::IOUtil::castSizeToInt64(section[DataLevel2].block_num) *
            tc::io::IOUtil::castSizeToInt64(section[DataLevel2].block_size)));
}
...
size_t pie::ctr::IvfcStream::read(byte_t *ptr, size_t count)
{
    ...
    int64_t end_aligned_offset = end_read_offset - offsetInBlock(end_read_offset) +
                                 (offsetInBlock(end_read_offset) ? mDataStreamBlockSize : 0x0);
    ...
    size_t continuous_begin_block =
        (continuous_block_num == 0) ? 0 : (offsetToBlock(begin_aligned_offset) + (size_t)read_partial_begin_block);
    ...
    */
    ...
    if (block_num == 0)
    {
        throw tc::InvalidOperationException(
            "pie::ctr::IvfcStream", "Invalid block number (0 blocks, would have returned before now if count==0)");
    }
    ...
    if (read_partial_begin_block)
    {
        ...
        // verify block
        if (validateLayerBlocksWithHashLayer(
                partial_block.data(), mDataStreamBlockSize, 1, getBlockHash(partial_begin_block)) == false)
        {
    ...
    if (continuous_block_num > 0)
    {
        ...
        // verify blocks
        if (validateLayerBlocksWithHashLayer(
                ptr + data_read_count, mDataStreamBlockSize, continuous_block_num,
                getBlockHash(continuous_begin_block)) == false)
        {
            throw tc::crypto::CryptoException("pie::ctr::IvfcStream", "Data layer block(s) failed hash validation.");
        }
    ...
    if (read_partial_end_block)
    {
        ...
        // verify block
        if (validateLayerBlocksWithHashLayer(
                partial_block.data(), mDataStreamBlockSize, 1, getBlockHash(partial_end_block)) == false)
        {
            throw tc::crypto::CryptoException("pie::ctr::IvfcStream", "Data layer block(s) failed hash validation.");
        }
...
int64_t pie::ctr::IvfcStream::seek(int64_t offset, tc::io::SeekOrigin origin)
{
    if (mDataStream == nullptr)
    {
        throw tc::ObjectDisposedException(
            mModuleLabel + "::seek()", "Failed to set stream position (stream is disposed)");
    }
...
void pie::ctr::IvfcStream::setLength(int64_t length)
{
    ...
    {
        throw tc::ObjectDisposedException(
            mModuleLabel + "::setLength()", "Failed to set stream length (stream is disposed)");
    }
...
bool pie::ctr::IvfcStream::validateLayerBlocksWithHashLayer(
    const byte_t *layer, size_t block_size, size_t block_num, const byte_t *hash_layer)
{
    ...
    for (size_t i = 0; i < block_num; i++)
    {
        ...
        // std::cout << "test hash: " << tc::cli::FormatUtil::formatBytesAsString(blk_hash_ptr, 32, true, ":") <<
        // std::endl; std::cout << "calc hash: " << tc::cli::FormatUtil::formatBytesAsString(mHash.data(), 32, true,
        // ":") << std::endl;
