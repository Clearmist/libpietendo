...
pie::ctr::ExeFsSnapshotGenerator::ExeFsSnapshotGenerator(
    const std::shared_ptr<tc::io::IStream> &stream, bool verify_hashes)
    : FileSystemSnapshot()
{
    ...
    if (stream->canRead() == false || stream->canSeek() == false)
    {
        throw tc::NotSupportedException(
            "pie::ctr::ExeFsSnapshotGenerator", "Input stream requires read/seek permissions.");
    }
    ...
    if (hdr.file_table[0].name[0] == 0 || hdr.file_table[0].offset.unwrap() != 0)
    {
        throw tc::ArgumentOutOfRangeException(
            "pie::ctr::ExeFsSnapshotGenerator", "ExeFsHeader is corrupted (Bad first entry).");
    }
    ...
    {
        // skip empty entry
        if (hdr.file_table[i].name.size() == 0 || hdr.file_table[i].size.unwrap() == 0)
        {
            continue;
        }
        ...
        {
            throw tc::ArgumentOutOfRangeException(
                "pie::ctr::ExeFsSnapshotGenerator", "ExeFs section had unexpected offset.");
        }
        ...
        pos = align<int64_t>(
            (static_cast<int64_t>(tmp.offset) + static_cast<int64_t>(tmp.size)),
            pie::ctr::ExeFsHeader::kExeFsSectionAlignSize);
    ...
    for (size_t i = 0; i < section.size(); i++)
    {
        if (section[i].size != 0)
        {
            ...
            {
                tmp.stream = std::make_shared<tc::io::SubStream>(tc::io::SubStream(
                    stream,
                    static_cast<int64_t>(section[i].offset) + static_cast<int64_t>(sizeof(pie::ctr::ExeFsHeader)),
                    static_cast<int64_t>(section[i].size)));
            }
