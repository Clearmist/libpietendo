    ...
    : FileSystemSnapshot()
{
    ...
    if (mBaseStream->canRead() == false || mBaseStream->canSeek() == false)
    {
        throw tc::NotSupportedException(
            "pie::ctr::CiaFsSnapshotGenerator", "Input stream requires read/seek permissions.");
    }
    ...
    if (hdr.format_version.unwrap() != pie::ctr::CiaHeader::FormatVersion_Default)
    {
        throw tc::ArgumentOutOfRangeException(
            "pie::ctr::CiaFsSnapshotGenerator", "CIA header had unexpected format version.");
    }
    ...
    section[Certificate].offset =
        align<int64_t>(section[Header].offset + section[Header].size, CiaHeader::kCiaSectionAlignment);
    section[Tik].size = hdr.ticket_size.unwrap();
    section[Tik].offset =
        align<int64_t>(section[Certificate].offset + section[Certificate].size, CiaHeader::kCiaSectionAlignment);
    ...
    {
        throw tc::ArgumentOutOfRangeException(
            "pie::ctr::CiaFsSnapshotGenerator", "CIA header had too large content section.");
    }
    ...
    section[Footer].offset =
        align<int64_t>(section[Content].offset + section[Content].size, CiaHeader::kCiaSectionAlignment);

    int64_t total_size = section[Footer].offset + section[Footer].size;
    if (stream->length() < total_size)
    {
        throw tc::ArgumentOutOfRangeException(
            "pie::ctr::CiaFsSnapshotGenerator", "Input stream is too small, given calculated CIA section geometry.");
    }
    ...
    if (section[Content].size != 0)
    {
        ...
        if (section[Tmd].size != 0)
        {
            ...
            {
                throw tc::ArgumentOutOfRangeException(
                    "pie::ctr::CiaFsSnapshotGenerator", "TMD was too large to read into memory.");
            }
            ...
            {
                throw tc::ArgumentOutOfRangeException(
                    "pie::ctr::CiaFsSnapshotGenerator", "TMD had unexpected size after reading.");
            }
            ...
            {
                throw tc::ArgumentOutOfRangeException(
                    "pie::ctr::CiaFsSnapshotGenerator", "TMD had unexpected signature type.");
            }
            if (tmd->head.version != 1)
            {
                throw tc::ArgumentOutOfRangeException(
                    "pie::ctr::CiaFsSnapshotGenerator", "TMD had unexpected format version.");
            }
            ...
            /*
            tc::crypto::GenerateSha2256Hash(hash.data(), (byte_t*)&tmd->sig.issuer,
            (size_t)((byte_t*)&tmd->v1Head.cmdGroups - (byte_t*)&tmd->sig.issuer)); if
            (tc::crypto::VerifyRsa2048Pkcs1Sha2256(tmd->sig.sig.data(), hash.data(), <rsa key here>))
            {
            ...
            */
            ...
            tc::crypto::GenerateSha2256Hash(
                hash.data(), (byte_t *)&tmd->v1Head.cmdGroups, sizeof(tmd->v1Head.cmdGroups));
            if (memcmp(hash.data(), tmd->v1Head.hash.data(), hash.size()) != 0)
            {
                throw tc::ArgumentOutOfRangeException(
                    "pie::ctr::CiaFsSnapshotGenerator", "TMD had invalid CMD group hash.");
            }
            ...
            std::cout << "  > TitleId:           " << std::hex << std::setfill('0') << std::setw(16) <<
            tmd->head.titleId.unwrap() << std::endl;
            */

            size_t cmd_table_num = tmd->v1Head.cmdGroups[0].nCmds.unwrap();
            if (tmd_data.size() !=
                (sizeof(pie::es::ESV1TitleMeta) + (cmd_table_num * sizeof(pie::es::ESV1ContentMeta))))
            {
                throw tc::ArgumentOutOfRangeException("pie::ctr::CiaFsSnapshotGenerator", "TMD had unexpected size");
            }

            tc::crypto::GenerateSha2256Hash(
                hash.data(), (byte_t *)&tmd->contents, cmd_table_num * sizeof(pie::es::ESV1ContentMeta));
            if (memcmp(hash.data(), tmd->v1Head.cmdGroups[0].groupHash.data(), hash.size()) != 0)
            {
                throw tc::ArgumentOutOfRangeException(
                    "pie::ctr::CiaFsSnapshotGenerator", "TMD had invalid CMD group[0] hash.");
            }
            ...
            for (size_t i = 0; i < cmd_table_num; i++)
            {
                ...
                "), "; std::cout << "index(" << std::hex << std::setfill('0') << std::setw(4) <<
                tmd->contents[i].index.unwrap() << "), "; std::cout << "type(" << std::hex << std::setfill('0') <<
                std::setw(4) << tmd->contents[i].type.unwrap() << "), "; std::cout << "size(" << std::hex <<
                std::setfill('0') << std::setw(16) << tmd->contents[i].size.unwrap() << "), "; std::cout << "" <<
                std::endl;
                */
                if (hdr.content_bitarray.test(tmd->contents[i].index.unwrap()))
                {
                    ...
                    int64_t content_size =
                        align<int64_t>(tmd->contents[i].size.unwrap(), CiaHeader::kCiaContentAlignment);
