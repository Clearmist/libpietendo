...
{
    return (mLayerInfo == other.mLayerInfo) && (mMasterHashList == other.mMasterHashList);
}
...
void pie::hac::HierarchicalIntegrityHeader::fromBytes(const byte_t *data, size_t len)
{
    ...
    if (hdr->type_id.unwrap() != (uint32_t)pie::hac::hierarchicalintegrity::TypeId_HAC_RomFs)
    {
        throw tc::ArgumentOutOfRangeException(
            kModuleName, fmt::format("Unsupported type id (0x{:x})", hdr->type_id.unwrap()));
    }
    ...
    {
        throw tc::ArgumentOutOfRangeException(
            kModuleName, fmt::format(
                             "Invalid layer count. (actual={:d}, expected={:d})", hdr->layer_num.unwrap(),
                             pie::hac::hierarchicalintegrity::kDefaultLayerNumForRomFs + 1));
    }

    // Get Sizes/Offsets
    size_t master_hash_offset = align(
        (sizeof(pie::hac::sHierarchicalIntegrityHeader) +
         sizeof(pie::hac::sHierarchicalIntegrityLayerInfo) * hdr->layer_num.unwrap()),
        pie::hac::hierarchicalintegrity::kHeaderAlignLen);
    ...
    // save layer info
    const pie::hac::sHierarchicalIntegrityLayerInfo *layer_info =
        (const pie::hac::sHierarchicalIntegrityLayerInfo
             *)(mRawBinary.data() + sizeof(pie::hac::sHierarchicalIntegrityHeader));
    for (size_t i = 0; i < hierarchicalintegrity::kDefaultLayerNumForRomFs; i++)
    {
        mLayerInfo.push_back(
            {layer_info[i].offset.unwrap(), layer_info[i].size.unwrap(),
             (int64_t)1 << (int64_t)layer_info[i].block_size.unwrap()});
    }
...
const std::vector<pie::hac::HierarchicalIntegrityHeader::sLayer> &
pie::hac::HierarchicalIntegrityHeader::getLayerInfo() const
{
    return mLayerInfo;
}
...
void pie::hac::HierarchicalIntegrityHeader::setMasterHashList(
    const std::vector<pie::hac::detail::sha256_hash_t> &master_hash_list)
{
