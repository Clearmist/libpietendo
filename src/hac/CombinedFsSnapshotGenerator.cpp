    ...
    : FileSystemSnapshot()
{
    ...
    for (auto mount = mount_point_info.begin(); mount != mount_point_info.end(); mount++)
    {
        ...
        for (auto oldDirEntry = mount->fs_meta.dir_entries.begin(); oldDirEntry != mount->fs_meta.dir_entries.end();
             oldDirEntry++)
        {
            ...
            for (auto oldDirChildFileName = oldDirEntry->dir_listing.file_list.begin();
                 oldDirChildFileName != oldDirEntry->dir_listing.file_list.end(); oldDirChildFileName++)
            {
