...
{
    return (mVersion == other.mVersion) && (mFsAccess == other.mFsAccess) &&
           (mContentOwnerIdList == other.mContentOwnerIdList) && (mSaveDataOwnerIdList == other.mSaveDataOwnerIdList);
}
...
void pie::hac::FileSystemAccessControl::toBytes()
{
    ...
    savedata.offset =
        (uint32_t)(content.offset + (content.size > 0 ? align<uint32_t>(content.size, (uint32_t)fac::kSectionAlignSize) : 0));
    ...
    // get total size
    size_t total_size = std::max<uint32_t>(
        std::max<uint32_t>(content.offset + content.size, savedata.offset + savedata.size),
        (uint32_t)align<size_t>(sizeof(sFacHeader), fac::kSectionAlignSize));
    ...
    tc::bn::le64<uint64_t> *content_owner_ids =
        (tc::bn::le64<uint64_t> *)(mRawBinary.data() + content.offset + sizeof(uint32_t));
    ...
    tc::bn::le64<uint64_t> *save_data_owner_ids =
        (tc::bn::le64<uint64_t>
             *)(mRawBinary.data() + savedata.offset + sizeof(uint32_t) + align<size_t>(mSaveDataOwnerIdList.size(), sizeof(uint32_t)));
...
void pie::hac::FileSystemAccessControl::fromBytes(const byte_t *data, size_t len)
{
    ...
    // get total size
    size_t total_size = std::max<size_t>(
        (size_t)std::max<uint32_t>(
            hdr.content_owner_ids.offset.unwrap() + hdr.content_owner_ids.size.unwrap(),
            hdr.save_data_owner_ids.offset.unwrap() + hdr.save_data_owner_ids.size.unwrap()),
        align<size_t>(sizeof(sFacHeader), fac::kSectionAlignSize));
    ...
    {
        size_t content_owner_id_num =
            ((tc::bn::le32<uint32_t> *)(mRawBinary.data() + hdr.content_owner_ids.offset.unwrap()))->unwrap();
        tc::bn::le64<uint64_t> *content_owner_ids =
            (tc::bn::le64<uint64_t> *)(mRawBinary.data() + hdr.content_owner_ids.offset.unwrap() + sizeof(uint32_t));
    ...
    {
        size_t save_data_owner_id_num =
            ((tc::bn::le32<uint32_t> *)(mRawBinary.data() + hdr.save_data_owner_ids.offset.unwrap()))->unwrap();
        byte_t *save_data_owner_id_accessibility_array =
            (mRawBinary.data() + hdr.save_data_owner_ids.offset.unwrap() + sizeof(uint32_t));
        tc::bn::le64<uint64_t> *save_data_owner_ids =
            (tc::bn::le64<uint64_t>
                 *)(mRawBinary.data() + hdr.save_data_owner_ids.offset.unwrap() + sizeof(uint32_t) + align(save_data_owner_id_num, fac::kSectionAlignSize));
        for (size_t i = 0; i < save_data_owner_id_num; i++)
        {
            mSaveDataOwnerIdList.push_back(
                {(fac::SaveDataOwnerIdAccessType)save_data_owner_id_accessibility_array[i],
                 save_data_owner_ids[i].unwrap()});
        }
    }
}
...
const std::vector<pie::hac::FileSystemAccessControl::sSaveDataOwnerId> &
pie::hac::FileSystemAccessControl::getSaveDataOwnerIdList() const
{
