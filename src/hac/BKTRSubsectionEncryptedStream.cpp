...
pie::hac::BKTRSubsectionEncryptedStream::BKTRSubsectionEncryptedStream() : mLength(0), mCurrentPos(0) {}
...
pie::hac::BKTRSubsectionEncryptedStream::BKTRSubsectionEncryptedStream(
    ...
    const pie::hac::sContentArchiveBucketInfo &bucket_info)
    : BKTRSubsectionEncryptedStream()
{
    ...
    const pie::hac::sAesCtrExStorageBlock *subsection_block =
        (const pie::hac::sAesCtrExStorageBlock *)subsection_block_raw.data();
    ...
    for (size_t i = 0; i < subsection_block->header.bucket_count; ++i)
    {
        ...
        for (size_t j = 0; j < subsection.header.entry_count; ++j)
        {
            ...
            if (reader == nullptr)
            {
                reader = std::make_shared<tc::crypto::Aes128CtrEncryptedStream>(
                    tc::crypto::Aes128CtrEncryptedStream(stream, key, new_counter));
            }
...
size_t pie::hac::BKTRSubsectionEncryptedStream::write(const byte_t *ptr, size_t count)
{
    throw tc::NotSupportedException(
        kClassName + "::write()", "write is not supported for BKTRSubsectionEncryptedStream");
}
...
{
    throw tc::NotSupportedException(
        kClassName + "::setLength()", "setLength is not supported for BKTRSubsectionEncryptedStream");
}
