...
{
    return (mProgramId == other.mProgramId) && (mFileSystemAccessControl == other.mFileSystemAccessControl) &&
...
void pie::hac::AccessControlInfo::toBytes()
{
    ...
    // get total size
    size_t total_size = (size_t)std::max<uint32_t>(
        std::max<uint32_t>(fac.offset + fac.size, sac.offset + sac.size), kc.offset + kc.size);
