...
pie::hac::ContentArchiveHeader::ContentArchiveHeader(const ContentArchiveHeader &other) : ContentArchiveHeader()
{
    *this = other;
}
...
{
    return (mFormatVersion == other.mFormatVersion) && (mDistributionType == other.mDistributionType) &&
           (mContentType == other.mContentType) && (mKeyGeneration == other.mKeyGeneration) &&
           (mSignatureKeyGeneration == other.mSignatureKeyGeneration) && (mKaekIndex == other.mKaekIndex) &&
           (mContentSize == other.mContentSize) && (mProgramId == other.mProgramId) &&
           (mContentIndex == other.mContentIndex) && (mSdkAddonVersion == other.mSdkAddonVersion) &&
           (mRightsId == other.mRightsId) && (mPartitionEntryList == other.mPartitionEntryList) &&
...
void pie::hac::ContentArchiveHeader::toBytes()
{
    ...
    for (size_t i = 0; i < mPartitionEntryList.size(); i++)
    {
        ...
        if (index >= nca::kPartitionNum)
            continue;

        hdr->partition_entry[index].start_blk.wrap(sizeToBlockNum(mPartitionEntryList[index].offset));
        hdr->partition_entry[index].end_blk.wrap(
            (sizeToBlockNum(mPartitionEntryList[index].offset + mPartitionEntryList[index].size)));
...
void pie::hac::ContentArchiveHeader::fromBytes(const byte_t *data, size_t len)
{
    ...
    case (nca::kNca3StructMagic):
        ...
        throw tc::ArgumentOutOfRangeException(
            kModuleName, "ContentArchive header corrupt (unrecognised header magic).");
    }
    ...
    for (size_t i = 0; i < nca::kPartitionNum; i++)
    {
        ...
        {
            mPartitionEntryList.push_back(
                {(byte_t)i, blockNumToSize(hdr->partition_entry[i].start_blk.unwrap()),
                 blockNumToSize(hdr->partition_entry[i].end_blk.unwrap() - hdr->partition_entry[i].start_blk.unwrap()),
                 hdr->fs_header_hash[i]});
        }
    }
}
...
const std::vector<pie::hac::ContentArchiveHeader::sPartitionEntry> &
pie::hac::ContentArchiveHeader::getPartitionEntryList() const
{
    return mPartitionEntryList;
}

void pie::hac::ContentArchiveHeader::setPartitionEntryList(
    const std::vector<pie::hac::ContentArchiveHeader::sPartitionEntry> &partition_entry_list)
{
...
uint32_t pie::hac::ContentArchiveHeader::sizeToBlockNum(int64_t real_size) const
{
    ...
    if (real_size >= kMaxValue)
    {
        throw tc::Exception(kModuleName, "Cannot convert address to block address");
    }
