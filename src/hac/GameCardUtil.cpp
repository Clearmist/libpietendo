...
void pie::hac::GameCardUtil::decryptXciHeader(pie::hac::sGcHeader *hdr, const byte_t *key)
{
    ...
    tc::crypto::DecryptAes128Cbc(
        hdr->extended_header.raw_data.data(), hdr->extended_header.raw_data.data(),
        hdr->extended_header.raw_data.size(), key, sizeof(pie::hac::detail::aes128_key_t), iv.data(), iv.size());
}
