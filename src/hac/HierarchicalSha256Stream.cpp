...
pie::hac::HierarchicalSha256Stream::HierarchicalSha256Stream()
    : mModuleLabel("pie::hac::HierarchicalSha256Stream"), mBaseStream(), mDataStreamBlockSize(0),
      mDataStreamLogicalLength(0), mDataStream(), mHashCache(), mHashCalc(new tc::crypto::Sha2256Generator())
{
}

pie::hac::HierarchicalSha256Stream::HierarchicalSha256Stream(
    const std::shared_ptr<tc::io::IStream> &stream, const pie::hac::HierarchicalSha256Header &hash_header)
    : HierarchicalSha256Stream()
{
    ...
    for (size_t i = 0; i < hash_layer.size(); i++)
    {
        ...
        if (!validateLayerBlocksWithHashLayer(
                tmp.data(), tmp.size(), block_size, align(tmp.size(), block_size) / block_size,
                i == 0 ? hash_header.getMasterHash().data() : hash_layer_data[i - 1].data()))
        {
    ...
    mDataStreamFinalBlockSize =
        offsetInBlock(mDataStreamLogicalLength) != 0 ? offsetInBlock(mDataStreamLogicalLength) : mDataStreamBlockSize;
    ...
    */
    ...
    if (block_num == 0)
    {
        tc::InvalidOperationException(
            "pie::hac::HierarchicalSha256Stream",
    ...
    if (read_partial_begin_block)
    {
        ...
        // verify block
        if (validateLayerBlocksWithHashLayer(
                partial_block.data(), getSizeOfBlock(partial_begin_block), mDataStreamBlockSize, 1,
                getBlockHash(partial_begin_block)) == false)
        {
            throw tc::crypto::CryptoException(
                "pie::hac::HierarchicalSha256Stream", "Data layer block(s) failed hash validation.");
        }
    ...
    if (continuous_block_num > 0)
    {
        ...
        // verify blocks
        if (validateLayerBlocksWithHashLayer(
                ptr + data_read_count, continuous_block_num * mDataStreamBlockSize, mDataStreamBlockSize,
                continuous_block_num, getBlockHash(continuous_begin_block)) == false)
        {
            throw tc::crypto::CryptoException(
                "pie::hac::HierarchicalSha256Stream", "Data layer block(s) failed hash validation.");
        }
    ...
    if (read_partial_end_block)
    {
        ...
        // verify block
        if (validateLayerBlocksWithHashLayer(
                partial_block.data(), getSizeOfBlock(partial_end_block), mDataStreamBlockSize, 1,
                getBlockHash(partial_end_block)) == false)
        {
            throw tc::crypto::CryptoException(
                "pie::hac::HierarchicalSha256Stream", "Data layer block(s) failed hash validation.");
        }
...
void pie::hac::HierarchicalSha256Stream::setLength(int64_t length)
{
    ...
    throw tc::NotSupportedException(
        mModuleLabel + "::setLength()", "setLength is not supported for HierarchicalSha256Stream");
}
...
bool pie::hac::HierarchicalSha256Stream::validateLayerBlocksWithHashLayer(
    const byte_t *layer, size_t layer_size, size_t block_size, size_t block_num, const byte_t *hash_layer)
{
