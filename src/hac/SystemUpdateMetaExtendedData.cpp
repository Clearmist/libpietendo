...
{
    return mFormatVersion == other.mFormatVersion && mFirmwareVariationInfo == other.mFirmwareVariationInfo;
}
...
void pie::hac::SystemUpdateMetaExtendedData::toBytes()
{
    if (mFormatVersion == 1)
    {
        mRawBinary = tc::ByteData(
            sizeof(sSystemUpdateMetaExtendedDataHeader) +
            sizeof(sFirmwareVariationInfo_v1) * mFirmwareVariationInfo.size());

        sSystemUpdateMetaExtendedDataHeader *hdr = (sSystemUpdateMetaExtendedDataHeader *)(mRawBinary.data() + 0x0);
        sFirmwareVariationInfo_v1 *variation_info =
            (sFirmwareVariationInfo_v1 *)(mRawBinary.data() + sizeof(sSystemUpdateMetaExtendedDataHeader));
        ...
        for (auto info = mFirmwareVariationInfo.begin(); info != mFirmwareVariationInfo.end(); info++, variation_info++)
        {
            if (info->meta.size() != 0)
            {
                throw tc::ArgumentException(
                    kModuleName, "Overriding ContentInfoMeta is not supported in format version 1");
            }
    ...
    else if (mFormatVersion == 2)
    {
        ...
        sSystemUpdateMetaExtendedDataHeader *hdr_ptr =
            (sSystemUpdateMetaExtendedDataHeader *)(mRawBinary.data() + header_offset);
        tc::bn::le32<uint32_t> *firmware_variation_ids_ptr =
            (tc::bn::le32<uint32_t> *)(mRawBinary.data() + firmware_variation_ids_offset);
        sFirmwareVariationInfo_v2 *variation_info_ptr =
            (sFirmwareVariationInfo_v2 *)(mRawBinary.data() + firmware_variation_info_offset);
             ...
             variation_info_itr++, firmware_variation_ids_ptr++, variation_info_ptr++)
        {
            ...
            for (auto meta_itr = variation_info_itr->meta.begin(); meta_itr != variation_info_itr->meta.end();
                 meta_itr++, meta_info_ptr++)
            {
                meta_itr->toBytes();
...
void pie::hac::SystemUpdateMetaExtendedData::fromBytes(const byte_t *bytes, size_t len)
{
    ...
    if (format_version == 1)
    {
        ...
        const sFirmwareVariationInfo_v1 *firmware_variation_info_ptr =
            (const sFirmwareVariationInfo_v1 *)(mRawBinary.data() + sizeof(sSystemUpdateMetaExtendedDataHeader));
        ...
        {
            mFirmwareVariationInfo.push_back(
                {firmware_variation_info_ptr->firmware_variation_id.unwrap(), std::vector<ContentMetaInfo>()});
        }
    }
    else if (format_version == 2)
    {
        ...
        firmware_variation_info_ptr =
            (const sFirmwareVariationInfo_v2 *)(mRawBinary.data() + firmware_variation_info_offset);
...
const std::vector<pie::hac::SystemUpdateMetaExtendedData::FirmwareVariationInfo> &
pie::hac::SystemUpdateMetaExtendedData::getFirmwareVariationInfo() const
{
    return mFirmwareVariationInfo;
}

void pie::hac::SystemUpdateMetaExtendedData::setFirmwareVariationInfo(
    const std::vector<pie::hac::SystemUpdateMetaExtendedData::FirmwareVariationInfo> &info)
{
