...
pie::hac::PartitionFsSnapshotGenerator::PartitionFsSnapshotGenerator(
    const std::shared_ptr<tc::io::IStream> &stream, ValidationMode validate_mode)
    : FileSystemSnapshot()
{
    ...
    if (stream->canRead() == false || stream->canSeek() == false)
    {
        throw tc::NotSupportedException(
            "pie::hac::PartitionFsSnapshotGenerator", "Input stream requires read/seek permissions.");
    }
    ...
    default:
        throw tc::ArgumentOutOfRangeException(
            "pie::hac::PartitionFsSnapshotGenerator", "sPfsHeader header is corrupt (Bad struct magic).");
    }

    // determine complete header size
    size_t pfs_full_header_size =
        sizeof(sPfsHeader) + file_entry_size * hdr.file_num.unwrap() + hdr.name_table_size.unwrap();
    ...
    {
        throw tc::ArgumentOutOfRangeException(
            "pie::hac::PartitionFsSnapshotGenerator", "sPfsHeader header is corrupt (file count was 0)");
    }

    if (hdr.name_table_size.unwrap() == 0)
    {
        throw tc::ArgumentOutOfRangeException(
            "pie::hac::PartitionFsSnapshotGenerator", "sPfsHeader header is corrupt (name table size was 0)");
    }
    ...
    for (size_t i = 0; i < section.size(); i++)
    {
        if (section[i].size != 0)
        {
            ...
            {
                tmp.stream =
                    std::make_shared<tc::io::SubStream>(tc::io::SubStream(stream, section[i].offset, section[i].size));
            }
