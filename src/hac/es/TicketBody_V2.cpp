...
{
    return (mIssuer == other.mIssuer) && (mEncTitleKey == other.mEncTitleKey) && (mEncType == other.mEncType) &&
           (mTicketVersion == other.mTicketVersion) && (mLicenseType == other.mLicenseType) &&
           (mPropertyFlags == other.mPropertyFlags) && (mReservedRegion == other.mReservedRegion) &&
           (mTicketId == other.mTicketId) && (mDeviceId == other.mDeviceId) && (mRightsId == other.mRightsId) &&
           (mAccountId == other.mAccountId) && (mSectTotalSize == other.mSectTotalSize) &&
           (mSectHeaderOffset == other.mSectHeaderOffset) && (mSectNum == other.mSectNum) &&
           (mSectEntrySize == other.mSectEntrySize);
}
...
void pie::hac::es::TicketBody_V2::fromBytes(const byte_t *bytes, size_t len)
{
    if (bytes == nullptr)
    {
        throw tc::ArgumentNullException(kModuleName, "bytes was null.");
    }
