...
{
    return (mIssuer == other.mIssuer) && (mSubject == other.mSubject) && (mCertId == other.mCertId) &&
           (mPublicKeyType == other.mPublicKeyType) && (mRsa4096PublicKey.n == other.mRsa4096PublicKey.n) &&
           (mRsa4096PublicKey.e == other.mRsa4096PublicKey.e) && (mRsa2048PublicKey.n == other.mRsa2048PublicKey.n) &&
           (mRsa2048PublicKey.e == other.mRsa2048PublicKey.e) && (mEcdsa240PublicKey.r == other.mEcdsa240PublicKey.r) &&
...
void pie::hac::es::CertificateBody::fromBytes(const byte_t *src, size_t size)
{
    if (src == nullptr)
    {
        throw tc::ArgumentNullException(kModuleName, "src was null.");
    }
    if (size < sizeof(sCertificateHeader))
    {
        tc::ArgumentOutOfRangeException(kModuleName, "src was too small.");
    }
    ...
    {
        throw tc::ArgumentOutOfRangeException(kModuleName, "src was too small.");
    }
    ...
    if (mPublicKeyType == cert::RSA4096)
    {
        const sRsa4096PublicKeyBlock *pubkey =
            (const sRsa4096PublicKeyBlock *)(mRawBinary.data() + sizeof(sCertificateHeader));
        mRsa4096PublicKey = tc::crypto::RsaPublicKey(pubkey->modulus.data(), pubkey->modulus.size());
    }
    else if (mPublicKeyType == cert::RSA2048)
    {
        const sRsa2048PublicKeyBlock *pubkey =
            (const sRsa2048PublicKeyBlock *)(mRawBinary.data() + sizeof(sCertificateHeader));
        mRsa2048PublicKey = tc::crypto::RsaPublicKey(pubkey->modulus.data(), pubkey->modulus.size());
    }
    else if (mPublicKeyType == cert::ECDSA240)
    {
        const sEcdsa240PublicKeyBlock *pubkey =
            (const sEcdsa240PublicKeyBlock *)(mRawBinary.data() + sizeof(sCertificateHeader));
...
{
    if (key.n.size() != cert::kRsa4096Size)
    {
        throw tc::ArgumentOutOfRangeException(kModuleName, "RSA4096 public key a bad modulus size");
    }
...
{
    if (key.n.size() != cert::kRsa2048Size)
    {
        throw tc::ArgumentOutOfRangeException(kModuleName, "RSA2048 public key a bad modulus size");
    }
