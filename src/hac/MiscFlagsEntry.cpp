...
pie::hac::MiscFlagsEntry::MiscFlagsEntry() : mCap(kCapId), mFlags(0) {}

pie::hac::MiscFlagsEntry::MiscFlagsEntry(const KernelCapabilityEntry &kernel_cap) : mCap(kCapId), mFlags(0)
{
    setKernelCapability(kernel_cap);
}

pie::hac::MiscFlagsEntry::MiscFlagsEntry(uint32_t flags) : mCap(kCapId), mFlags(0)
{
    setFlags(flags);
}
...
void pie::hac::MiscFlagsEntry::setFlags(uint32_t flags)
{
    if ((flags & ~kMaxVal) != 0)
    {
        throw tc::ArgumentOutOfRangeException(
            kModuleName, "Illegal MiscFlag bits set. (settable bits: 0-14 inclusive)");
    }
