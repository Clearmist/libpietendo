...
void pie::hac::ContentArchiveUtil::decryptContentArchiveHeader(
    const byte_t *src, byte_t *dst, const detail::aes128_xtskey_t &key)
{
    // init aes-xts
    ...
    bool useNca2SectorIndex =
        ((pie::hac::sContentArchiveHeader *)(raw_hdr))->st_magic.unwrap() == pie::hac::nca::kNca2StructMagic;

    // decrypt whole header
    for (size_t i = 0; i < pie::hac::nca::kHeaderSectorNum; i++)
    {
        enc.decrypt(
            dst + sectorToOffset(i), src + sectorToOffset(i), pie::hac::nca::kSectorSize,
            (i >= 2 && useNca2SectorIndex) ? 0 : i);
    }
}
...
std::string
pie::hac::ContentArchiveUtil::getProgramContentParititionIndexAsString(pie::hac::nca::ProgramContentPartitionIndex val)
{
    std::string str;
...
std::string
pie::hac::ContentArchiveUtil::getKeyAreaEncryptionKeyIndexAsString(pie::hac::nca::KeyAreaEncryptionKeyIndex val)
{
    std::string str;
