...
void pie::hac::PartitionFsHeader::toBytes()
{
    ...
    int64_t pfs_header_size = align<int64_t>(
        tc::io::IOUtil::castSizeToInt64(sizeof(sPfsHeader)) +
            tc::io::IOUtil::castSizeToInt64(getFileEntrySize(mFsType) * mFileList.size()) + name_table_size,
        tc::io::IOUtil::castSizeToInt64(pfs::kHeaderAlign));
    ...
    if (mFsType == TYPE_PFS0)
    {
        ...
        for (size_t i = 0; i < mFileList.size(); i++)
        {
            ...
            {
                throw tc::Exception(
                    kModuleName, "raw_name_table_pos could not be safely serialised as it was too large.");
            }
    ...
    else if (mFsType == TYPE_HFS0)
    {
        ...
        char *raw_name_table =
            (char *)(mRawBinary.data() + sizeof(sPfsHeader) + sizeof(sHashedPfsFile) * mFileList.size());
        size_t raw_name_table_pos = 0;
        ...
        for (size_t i = 0; i < mFileList.size(); i++)
        {
            ...
            {
                throw tc::Exception(
                    kModuleName, "hash_protected_size could not be safely serialised as it was too large.");
            }
...
void pie::hac::PartitionFsHeader::fromBytes(const byte_t *data, size_t len)
{
    ...
    size_t pfs_full_header_size =
        sizeof(sPfsHeader) + getFileEntrySize(fs_type) * hdr->file_num.unwrap() + hdr->name_table_size.unwrap();
    ...
    if (mFsType == TYPE_PFS0)
    {
        ...
        const char *raw_name_table =
            (const char *)(mRawBinary.data() + sizeof(sPfsHeader) + sizeof(sPfsFile) * hdr->file_num.unwrap());
    ...
    else if (mFsType == TYPE_HFS0)
    {
        ...
        const char *raw_name_table =
            (const char *)(mRawBinary.data() + sizeof(sPfsHeader) + sizeof(sHashedPfsFile) * hdr->file_num.unwrap());
        ...
        for (size_t i = 0; i < hdr->file_num.unwrap(); i++)
        {
            mFileList.push_back(
                {std::string(raw_name_table + raw_files[i].name_offset.unwrap()),
                 raw_files[i].data_offset.unwrap() + tc::io::IOUtil::castSizeToInt64(pfs_full_header_size),
                 raw_files[i].size.unwrap(), int64_t(raw_files[i].hash_protected_size.unwrap()), raw_files[i].hash});
        }
    }
}
...
void pie::hac::PartitionFsHeader::addFile(
    const std::string &name, int64_t size, int64_t hash_protected_size, const pie::hac::detail::sha256_hash_t &hash)
{
    if (hash_protected_size >= int64_t(std::numeric_limits<uint32_t>::max()))
    {
        throw tc::ArgumentOutOfRangeException(
            kModuleName + "::addFile()", "hash_protected_size cannot exceed 0xffffffff (max for uint32_t)");
    }
