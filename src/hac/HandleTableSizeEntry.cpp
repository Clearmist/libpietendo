...
pie::hac::HandleTableSizeEntry::HandleTableSizeEntry() : mCap(kCapId), mHandleTableSize(0) {}

pie::hac::HandleTableSizeEntry::HandleTableSizeEntry(const KernelCapabilityEntry &kernel_cap)
    : mCap(kCapId), mHandleTableSize(0)
{
    setKernelCapability(kernel_cap);
}

pie::hac::HandleTableSizeEntry::HandleTableSizeEntry(uint16_t size) : mCap(kCapId), mHandleTableSize(0)
{
