...
{
    return (mTitleId == other.mTitleId) && (mTitleVersion == other.mTitleVersion) && (mType == other.mType) &&
           (mAttribute == other.mAttribute) && (mStorageId == other.mStorageId) &&
           (mContentInstallType == other.mContentInstallType) && (mInstallState == other.mInstallState) &&
           (mRequiredDownloadSystemVersion == other.mRequiredDownloadSystemVersion) &&
           (mApplicationMetaExtendedHeader == other.mApplicationMetaExtendedHeader) &&
           (mPatchMetaExtendedHeader == other.mPatchMetaExtendedHeader) &&
           (mAddOnContentMetaExtendedHeader == other.mAddOnContentMetaExtendedHeader) &&
           (mDeltaMetaExtendedHeader == other.mDeltaMetaExtendedHeader) && (mContentInfo == other.mContentInfo) &&
           (mContentMetaInfo == other.mContentMetaInfo) && (mPatchMetaExtendedData == other.mPatchMetaExtendedData) &&
           (mDeltaMetaExtendedData == other.mDeltaMetaExtendedData) &&
           (mSystemUpdateMetaExtendedData == other.mSystemUpdateMetaExtendedData) &&
...
void pie::hac::ContentMeta::fromBytes(const byte_t *data, size_t len)
{
    ...
    {
        const sContentMetaInfo *info =
            (const sContentMetaInfo
                 *)(data + getContentMetaInfoOffset(hdr->exhdr_size.unwrap(), hdr->content_count.unwrap()));
    ...
    if (exdata_size > 0)
    {
        ...
        case (cnmt::ContentMetaType_Patch):
            ...
            memcpy(
                mPatchMetaExtendedData.data(),
                data + getExtendedDataOffset(
                           hdr->exhdr_size.unwrap(), hdr->content_count.unwrap(), hdr->content_meta_count.unwrap()),
                exdata_size);
            break;
        case (cnmt::ContentMetaType_Delta):
            mDeltaMetaExtendedData = tc::ByteData(exdata_size);
            memcpy(
                mDeltaMetaExtendedData.data(),
                data + getExtendedDataOffset(
                           hdr->exhdr_size.unwrap(), hdr->content_count.unwrap(), hdr->content_meta_count.unwrap()),
                exdata_size);
            break;
        case (cnmt::ContentMetaType_SystemUpdate):
            mSystemUpdateMetaExtendedData.fromBytes(
                data + getExtendedDataOffset(
                           hdr->exhdr_size.unwrap(), hdr->content_count.unwrap(), hdr->content_meta_count.unwrap()),
                exdata_size);
            break;
    ...
    memcpy(
        ...
        data +
            getDigestOffset(
                hdr->exhdr_size.unwrap(), hdr->content_count.unwrap(), hdr->content_meta_count.unwrap(), exdata_size),
        mDigest.size());
...
void pie::hac::ContentMeta::validateBinary(const byte_t *data, size_t len) const
{
    ...
    if (len < getTotalSize(
                  hdr->exhdr_size.unwrap(), hdr->content_count.unwrap(), hdr->content_meta_count.unwrap(),
                  getExtendedDataSize((cnmt::ContentMetaType)hdr->type, data + getExtendedHeaderOffset())))
    {
        throw tc::ArgumentOutOfRangeException(kModuleName, "Binary too small");
    }
}