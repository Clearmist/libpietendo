...
pie::hac::Meta::Meta(const Meta &other) : Meta()
{
    *this = other;
}
...
bool pie::hac::Meta::operator==(const Meta &other) const
{
    return (mAccessControlInfoDescKeyGeneration == other.mAccessControlInfoDescKeyGeneration) &&
           ...
           (mProcessAddressSpace == other.mProcessAddressSpace) &&
           (mOptimizeMemoryAllocationFlag == other.mOptimizeMemoryAllocationFlag) &&
           (mMainThreadPriority == other.mMainThreadPriority) && (mMainThreadCpuId == other.mMainThreadCpuId) &&
           (mSystemResourceSize == other.mSystemResourceSize) && (mVersion == other.mVersion) &&
           (mMainThreadStackSize == other.mMainThreadStackSize) && (mName == other.mName) &&
           (mProductCode == other.mProductCode) && (mAccessControlInfo == other.mAccessControlInfo) &&
           (mAccessControlInfoDesc == other.mAccessControlInfoDesc);
}
...
void pie::hac::Meta::toBytes()
{
    ...
    size_t total_size = std::max<size_t>(
        std::max<size_t>(acid.offset + acid.size, aci.offset + aci.size),
        align<size_t>(sizeof(sMetaHeader), meta::kSectionAlignSize));
    ...
    {
        memcpy(
            mRawBinary.data() + aci.offset, mAccessControlInfo.getBytes().data(), mAccessControlInfo.getBytes().size());
    }
    if (mAccessControlInfoDesc.getBytes().size() > 0)
    {
        memcpy(
            mRawBinary.data() + acid.offset, mAccessControlInfoDesc.getBytes().data(),
            mAccessControlInfoDesc.getBytes().size());
    }
}
...
void pie::hac::Meta::fromBytes(const byte_t *data, size_t len)
{
    ...
    size_t total_size = std::max<size_t>(
        (size_t)std::max<uint32_t>(
            hdr.aci_desc.offset.unwrap() + hdr.aci_desc.size.unwrap(), hdr.aci.offset.unwrap() + hdr.aci.size.unwrap()),
        sizeof(sMetaHeader));
