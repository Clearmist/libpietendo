...
pie::hac::GameCardFsSnapshotGenerator::GameCardFsSnapshotGenerator(
    const std::shared_ptr<tc::io::IStream> &stream, size_t root_header_size, ValidationMode validate_mode)
    : FileSystemSnapshot()
{
    ...
    if (stream->canRead() == false || stream->canSeek() == false)
    {
        throw tc::NotSupportedException(
            "pie::hac::GameCardFsSnapshotGenerator", "Input stream requires read/seek permissions.");
    }
    ...
    for (auto dirItr = dir_list.begin(); dirItr != dir_list.end(); dirItr++)
    {
        ...
        for (auto fileItr = dirItr->file_list.begin(); fileItr != dirItr->file_list.end(); fileItr++)
        {
            ...
            {
                file_tmp.stream =
                    std::make_shared<tc::io::SubStream>(tc::io::SubStream(stream, fileItr->offset, fileItr->size));
            }
