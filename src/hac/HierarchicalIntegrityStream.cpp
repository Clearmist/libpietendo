...
pie::hac::HierarchicalIntegrityStream::HierarchicalIntegrityStream()
    : mModuleLabel("pie::hac::HierarchicalIntegrityStream"), mBaseStream(), mDataStreamBlockSize(0),
      mDataStreamLogicalLength(0), mDataStream(), mHashCache(), mHashCalc(new tc::crypto::Sha2256Generator())
{
}

pie::hac::HierarchicalIntegrityStream::HierarchicalIntegrityStream(
    const std::shared_ptr<tc::io::IStream> &stream, const pie::hac::HierarchicalIntegrityHeader &hash_header)
    : HierarchicalIntegrityStream()
{
    ...
    if (mBaseStream->canRead() == false)
    {
        throw tc::InvalidOperationException(
            "pie::hac::HierarchicalIntegrityStream", "stream does not support reading.");
    }
    if (mBaseStream->canSeek() == false)
    {
        throw tc::InvalidOperationException(
            "pie::hac::HierarchicalIntegrityStream", "stream does not support seeking.");
    }

    // create raw master hash table
    tc::ByteData master_hash_data =
        tc::ByteData(hash_header.getMasterHashList().size() * sizeof(pie::hac::detail::sha256_hash_t));
    ...
    for (size_t i = 0; i < hash_header.getLayerInfo().size(); i++)
    {
        ...
        if (tc::is_int64_t_too_large_for_size_t(raw_layer.block_size) ||
            (raw_layer.block_size & (raw_layer.block_size - 1)) != 0)
        {
        ...
        {
            data_layer = {
                raw_layer.offset, raw_layer.size, size_t(raw_layer.block_size),
                size_t((raw_layer.size + (raw_layer.block_size - 1)) / raw_layer.block_size)};
        }
        else
        {
            ...
            {
                throw tc::Exception(
                    mModuleLabel, fmt::format("IVFC hash layer {:d} was too large to read into memory", i));
            }

            hash_layer.push_back(
                {raw_layer.offset, raw_layer.size, size_t(raw_layer.block_size),
                 size_t((raw_layer.size + (raw_layer.block_size - 1)) / raw_layer.block_size)});
        }
    }
    ...
    for (size_t i = 0; i < hash_layer.size(); i++)
    {
        ...
        if (!validateLayerBlocksWithHashLayer(
                tmp.data(), hash_layer[i].block_size, hash_layer[i].block_num,
                i == 0 ? master_hash_data.data() : hash_layer_data[i - 1].data()))
        {
            throw tc::crypto::CryptoException(
                mModuleLabel, fmt::format("Hash layer {:d} failed hash verification.", i));
        }
    ...
    int64_t data_layer_physical_size =
        mBaseStream->length() > data_layer.offset ? (mBaseStream->length() - data_layer.offset) : 0;
    // fmt::print("make IVFC data stream:\n offset=0x{:x}\n size=0x{:x}\n", data_layer.offset,
    // mDataStreamLogicalLength);
    mDataStream = std::shared_ptr<tc::io::SubStream>(
        new tc::io::SubStream(mBaseStream, data_layer.offset, data_layer_physical_size));
    ...
    */
    ...
    if (block_num == 0)
    {
        tc::InvalidOperationException(
            "pie::hac::HierarchicalIntegrityStream",
            "Invalid block number (0 blocks, would have returned before now if count==0)");
    }

    if (block_num < continuous_block_num)
    {
        tc::InvalidOperationException(
            "pie::hac::HierarchicalIntegrityStream", "Invalid block number (underflow error)");
    }
    ...
    if (read_partial_begin_block)
    {
        ...
        {
            throw tc::crypto::CryptoException(
                "pie::hac::HierarchicalIntegrityStream", "Data layer block(s) failed hash validation.");
        }
...
size_t pie::hac::HierarchicalIntegrityStream::write(const byte_t *ptr, size_t count)
{
    throw tc::NotSupportedException(
        mModuleLabel + "::write()", "write is not supported for HierarchicalIntegrityStream");
}
...
void pie::hac::HierarchicalIntegrityStream::setLength(int64_t length)
{
    ...
    throw tc::NotSupportedException(
        mModuleLabel + "::setLength()", "setLength is not supported for HierarchicalIntegrityStream");
}
...
bool pie::hac::HierarchicalIntegrityStream::validateLayerBlocksWithHashLayer(
