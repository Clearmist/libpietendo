...
{
    return (mTitle == other.mTitle) && (mIsbn == other.mIsbn) && (mStartupUserAccount == other.mStartupUserAccount) &&
           (mUserAccountSwitchLock == other.mUserAccountSwitchLock) &&
           (mAddOnContentRegistrationType == other.mAddOnContentRegistrationType) && (mAttribute == other.mAttribute) &&
           (mSupportedLanguage == other.mSupportedLanguage) && (mParentalControl == other.mParentalControl) &&
           (mScreenshot == other.mScreenshot) && (mVideoCapture == other.mVideoCapture) &&
           (mDataLossConfirmation == other.mDataLossConfirmation) && (mPlayLogPolicy == other.mPlayLogPolicy) &&
           (mPresenceGroupId == other.mPresenceGroupId) && (mRatingAge == other.mRatingAge) &&
           (mDisplayVersion == other.mDisplayVersion) && (mAddOnContentBaseId == other.mAddOnContentBaseId) &&
           (mSaveDataOwnerId == other.mSaveDataOwnerId) &&
           (mUserAccountSaveDataSize == other.mUserAccountSaveDataSize) &&
           (mDeviceSaveDataSize == other.mDeviceSaveDataSize) &&
           (mBcatDeliveryCacheStorageSize == other.mBcatDeliveryCacheStorageSize) &&
           (mApplicationErrorCodeCategory == other.mApplicationErrorCodeCategory) &&
           (mLocalCommunicationId == other.mLocalCommunicationId) && (mLogoType == other.mLogoType) &&
           (mLogoHandling == other.mLogoHandling) &&
           (mRuntimeAddOnContentInstall == other.mRuntimeAddOnContentInstall) &&
           (mRuntimeParameterDelivery == other.mRuntimeParameterDelivery) &&
           (mAppropriateAgeForChina == other.mAppropriateAgeForChina) && (mCrashReport == other.mCrashReport) &&
           (mHdcp == other.mHdcp) && (mSeedForPsuedoDeviceId == other.mSeedForPsuedoDeviceId) &&
           (mBcatPassphase == other.mBcatPassphase) && (mStartupUserAccountOption == other.mStartupUserAccountOption) &&
           (mUserAccountSaveDataMax == other.mUserAccountSaveDataMax) &&
           (mDeviceSaveDataMax == other.mDeviceSaveDataMax) && (mTemporaryStorageSize == other.mTemporaryStorageSize) &&
           (mCacheStorageSize == other.mCacheStorageSize) &&
           (mCacheStorageDataAndJournalSizeMax == other.mCacheStorageDataAndJournalSizeMax) &&
           (mCacheStorageIndexMax == other.mCacheStorageIndexMax) && (mRuntimeUpgrade == other.mRuntimeUpgrade) &&
           (mSupportingLimitedApplicationLicenses == other.mSupportingLimitedApplicationLicenses) &&
           (mPlayLogQueryableApplicationId == other.mPlayLogQueryableApplicationId) &&
           (mPlayLogQueryCapability == other.mPlayLogQueryCapability) && (mRepair == other.mRepair) &&
           (mProgramIndex == other.mProgramIndex) &&
           (mRequiredNetworkServiceLicenseOnLaunch == other.mRequiredNetworkServiceLicenseOnLaunch) &&
           (mApplicationErrorCodePrefix == other.mApplicationErrorCodePrefix) && (mAcdIndex == other.mAcdIndex) &&
           (mApparentPlatform == other.mApparentPlatform) &&
           (mNeighborDetectionClientConfiguration == other.mNeighborDetectionClientConfiguration) &&
           (mJitConfiguration == other.mJitConfiguration) && (mPlayReportPermission == other.mPlayReportPermission) &&
           (mCrashScreenshotForProd == other.mCrashScreenshotForProd) &&
           (mCrashScreenshotForDev == other.mCrashScreenshotForDev) &&
           (mContentsAvailabilityTransitionPolicy == other.mContentsAvailabilityTransitionPolicy) &&
           (mAccessibleLaunchRequiredVersionApplicationId == other.mAccessibleLaunchRequiredVersionApplicationId) &&
           (mAlbumFileExport == other.mAlbumFileExport) && (mPlatformSpecificRegion == other.mPlatformSpecificRegion);
}
...
void pie::hac::ApplicationControlProperty::toBytes()
{
    ...
    for (size_t i = 0; i < mAccessibleLaunchRequiredVersionApplicationId.size() &&
                       i < nacp::kMaxAccessibleLaunchRequiredVersionApplicationIdCount;
         i++)
    {
        nacp->accessible_launch_required_verison.application_id[i].wrap(
            mAccessibleLaunchRequiredVersionApplicationId[i]);
    }
    ...
    memcpy(
        nacp->platform_specific_region.data(), mPlatformSpecificRegion.data.data(),
        nacp->platform_specific_region.size());
    ...
    serialiseGroupConfig(
        mNeighborDetectionClientConfiguration.send_data_configuration,
        nacp->neighbour_detection_client_configuration.send_group_configuration);
    for (size_t i = 0; i < nacp::kReceivableGroupConfigurationCount; i++)
    {
        serialiseGroupConfig(
            mNeighborDetectionClientConfiguration.receivable_data_configuration[i],
            nacp->neighbour_detection_client_configuration.receivable_group_configuration[i]);
    }
}
...
void pie::hac::ApplicationControlProperty::fromBytes(const byte_t *bytes, size_t len)
{
    ...
    mContentsAvailabilityTransitionPolicy =
        nacp::ContentsAvailabilityTransitionPolicy(nacp->contents_availability_transition_policy);
    ...
    for (size_t i = 0; i < nacp::kMaxAccessibleLaunchRequiredVersionApplicationIdCount; i++)
    {
        if (nacp->accessible_launch_required_verison.application_id[i].unwrap() != 0)
            mAccessibleLaunchRequiredVersionApplicationId.push_back(
                nacp->accessible_launch_required_verison.application_id[i].unwrap());
    }
    memcpy(
        mPlatformSpecificRegion.data.data(), nacp->platform_specific_region.data(),
        mPlatformSpecificRegion.data.size());
    ...
    // jit configuration
    mJitConfiguration.is_enabled =
        nacp->jit_configuration.jit_configuration_flag.test((size_t)nacp::JitConfigurationFlag_Enabled);
...
pie::hac::nacp::AddOnContentRegistrationType
pie::hac::ApplicationControlProperty::getAddOnContentRegistrationType() const
{
...
const std::vector<pie::hac::ApplicationControlProperty::sRating> &
pie::hac::ApplicationControlProperty::getRatingAge() const
{
    return mRatingAge;
}
...
const pie::hac::ApplicationControlProperty::sStorageSize &
pie::hac::ApplicationControlProperty::getUserAccountSaveDataSize() const
{
...
const pie::hac::ApplicationControlProperty::sStorageSize &
pie::hac::ApplicationControlProperty::getDeviceSaveDataSize() const
{
...
const std::vector<pie::hac::nacp::StartupUserAccountOptionFlag> &
pie::hac::ApplicationControlProperty::getStartupUserAccountOption() const
{
    return mStartupUserAccountOption;
}

void pie::hac::ApplicationControlProperty::setStartupUserAccountOption(
    const std::vector<nacp::StartupUserAccountOptionFlag> &var)
{
    mStartupUserAccountOption = var;
}

const pie::hac::ApplicationControlProperty::sStorageSize &
pie::hac::ApplicationControlProperty::getUserAccountSaveDataMax() const
{
...
const pie::hac::ApplicationControlProperty::sStorageSize &
pie::hac::ApplicationControlProperty::getDeviceSaveDataMax() const
{
...
const pie::hac::ApplicationControlProperty::sStorageSize &
pie::hac::ApplicationControlProperty::getCacheStorageSize() const
{
...
const std::vector<pie::hac::nacp::SupportingLimitedApplicationLicensesFlag> &
pie::hac::ApplicationControlProperty::getSupportingLimitedApplicationLicenses() const
{
    return mSupportingLimitedApplicationLicenses;
}

void pie::hac::ApplicationControlProperty::setSupportingLimitedApplicationLicenses(
    const std::vector<nacp::SupportingLimitedApplicationLicensesFlag> &var)
{
...
const std::vector<pie::hac::nacp::RequiredNetworkServiceLicenseOnLaunchFlag> &
pie::hac::ApplicationControlProperty::getRequiredNetworkServiceLicenseOnLaunch() const
{
    return mRequiredNetworkServiceLicenseOnLaunch;
}

void pie::hac::ApplicationControlProperty::setRequiredNetworkServiceLicenseOnLaunch(
    const std::vector<nacp::RequiredNetworkServiceLicenseOnLaunchFlag> &var)
{
...
const pie::hac::ApplicationControlProperty::sNeighborDetectionClientConfiguration &
pie::hac::ApplicationControlProperty::getNeighborDetectionClientConfiguration() const
{
    return mNeighborDetectionClientConfiguration;
}

void pie::hac::ApplicationControlProperty::setNeighborDetectionClientConfiguration(
    const sNeighborDetectionClientConfiguration &var)
{
    mNeighborDetectionClientConfiguration = var;
}

const pie::hac::ApplicationControlProperty::sJitConfiguration &
pie::hac::ApplicationControlProperty::getJitConfiguration() const
{
...
pie::hac::nacp::ContentsAvailabilityTransitionPolicy
pie::hac::ApplicationControlProperty::getContentsAvailabilityTransitionPolicy() const
{
    return mContentsAvailabilityTransitionPolicy;
}

void pie::hac::ApplicationControlProperty::setContentsAvailabilityTransitionPolicy(
    nacp::ContentsAvailabilityTransitionPolicy var)
{
    mContentsAvailabilityTransitionPolicy = var;
}

const std::vector<uint64_t> &
pie::hac::ApplicationControlProperty::getAccessibleLaunchRequiredVersionApplicationId() const
{
    return mAccessibleLaunchRequiredVersionApplicationId;
}

void pie::hac::ApplicationControlProperty::setAccessibleLaunchRequiredVersionApplicationId(
    const std::vector<uint64_t> &var)
{
...
void pie::hac::ApplicationControlProperty::serialiseGroupConfig(
    ...
    sApplicationControlProperty::sNeighborDetectionClientConfiguration::sGroupConfiguration &serialised)
{
    if (logical.isNull())
    {
        memset(
            &serialised, 0,
            sizeof(sApplicationControlProperty::sNeighborDetectionClientConfiguration::sGroupConfiguration));
        return;
    }
...
void pie::hac::ApplicationControlProperty::parseGroupConfig(
