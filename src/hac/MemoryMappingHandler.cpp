...
pie::hac::MemoryMappingHandler::MemoryMappingHandler() : mIsSet(false) {}
...
{
    return (mIsSet == other.mIsSet) && (mMemRange == other.mMemRange) && (mMemPage == other.mMemPage);
}
...
void pie::hac::MemoryMappingHandler::importKernelCapabilityList(const std::vector<KernelCapabilityEntry> &caps)
{
    ...
    for (size_t i = 0; i < entries.size();)
    {
        ...
        if (entries[i].isMultiplePages())
        {
            ...
            // add to list
            mMemRange.push_back(
                {entries[i].getPage(), entries[i + 1].getPage(),
                 entries[i].getFlag() ? kc::MemoryPermission_Ro : kc::MemoryPermission_Rw,
                 entries[i + 1].getFlag() ? kc::MappingType_Static : kc::MappingType_Io});
...
const std::vector<pie::hac::MemoryMappingHandler::sMemoryMapping> &
pie::hac::MemoryMappingHandler::getIoMemoryMaps() const
{
    return mMemPage;
}