...
bool pie::hac::AccessControlInfoDesc::operator==(const AccessControlInfoDesc &other) const
{
    return (mContentArchiveHeaderSignature2Key.n == other.mContentArchiveHeaderSignature2Key.n) &&
           ...
           (mProductionFlag == other.mProductionFlag) && (mUnqualifiedApprovalFlag == other.mUnqualifiedApprovalFlag) &&
           (mMemoryRegion == other.mMemoryRegion) && (mProgramIdRestrict == other.mProgramIdRestrict) &&
           (mFileSystemAccessControl == other.mFileSystemAccessControl) &&
           (mServiceAccessControl == other.mServiceAccessControl) && (mKernelCapabilities == other.mKernelCapabilities);
}
...
void pie::hac::AccessControlInfoDesc::toBytes()
{
    ...
    // get total size
    size_t total_size =
        std::max<uint32_t>(std::max<uint32_t>(fac.offset + fac.size, sac.offset + sac.size), kc.offset + kc.size);
    ...
    {
        memcpy(
            hdr->nca_rsa_signature2_modulus.data(), mContentArchiveHeaderSignature2Key.n.data(),
            hdr->nca_rsa_signature2_modulus.size());
    }
    else
    {
...
void pie::hac::AccessControlInfoDesc::fromBytes(const byte_t *data, size_t len)
{
    ...
    size_t total_size = std::max<uint32_t>(
        std::max<uint32_t>(
            hdr.fac.offset.unwrap() + hdr.fac.size.unwrap(), hdr.sac.offset.unwrap() + hdr.sac.size.unwrap()),
        hdr.kc.offset.unwrap() + hdr.kc.size.unwrap());
    ...
    // save variables
    mContentArchiveHeaderSignature2Key =
        tc::crypto::RsaPublicKey(hdr.nca_rsa_signature2_modulus.data(), hdr.nca_rsa_signature2_modulus.size());
...
void pie::hac::AccessControlInfoDesc::generateSignature(const tc::crypto::RsaKey &key)
{
    ...
    tc::crypto::GenerateSha2256Hash(
        hash.data(), mRawBinary.data() + sizeof(detail::rsa2048_signature_t),
        mRawBinary.size() - sizeof(detail::rsa2048_signature_t));
