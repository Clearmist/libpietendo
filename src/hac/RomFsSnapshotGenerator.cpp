      ...
      mFileEntryTable()
{
    ...
    if (mBaseStream->canRead() == false || mBaseStream->canSeek() == false)
    {
        throw tc::NotSupportedException(
            "pie::hac::RomFsSnapshotGenerator", "Input stream requires read/seek permissions.");
    }
    ...
    fmt::print(" > header_size = 0x{:04x} (expected: 0x{:04x})\n", hdr.header_size.unwrap(),
    sizeof(pie::hac::sRomfsHeader)); fmt::print(" > dir_hash_bucket\n"); fmt::print("   > offset =    0x{:04x}\n",
    hdr.dir_hash_bucket.offset.unwrap()); fmt::print("   > size =      0x{:04x}\n", hdr.dir_hash_bucket.size.unwrap());
    fmt::print(" > dir_entry\n");
    ...
    fmt::print(" > data_offset = 0x{:04x} (expected: 0x{:04x})\n", hdr.data_offset.unwrap(),
    align<int64_t>(hdr.header_size.unwrap(), pie::hac::romfs::kRomfsHeaderAlign));
    */
    ...
    for (uint32_t v_addr = 0; v_addr < mDirEntryTable.size();)
    {
            ...
            uint32_t total_size = sizeof(pie::hac::sRomfsDirEntry) +
    align<uint32_t>(getDirEntry(v_addr)->name_size.unwrap(), 4); fmt::print(" > entry_size:   0x{:x}\n", total_size);
    ...
    */
    ...
    for (uint32_t v_addr = 0; v_addr < mFileEntryTable.size();)
    {
            ...
            uint32_t total_size = sizeof(pie::hac::sRomfsFileEntry) +
    align<uint32_t>(getFileEntry(v_addr)->name_size.unwrap(), 4); fmt::print(" > entry_size:   0x{:x}\n", total_size);
    ...
    */
    ...
    for (uint32_t v_addr = 0; size_t(v_addr) < mDirEntryTable.size();)
    {
        ...
        uint32_t entry_total_size =
            sizeof(pie::hac::sRomfsDirEntry) + align<uint32_t>(getDirEntry(v_addr)->name_size.unwrap(), 4);

        if (getDirEntry(v_addr)->sibling_offset.unwrap() < v_addr)
        {
            throw tc::InvalidOperationException(
                "pie::hac::RomFsSnapshotGenerator", "Possibly corrupted directory entry");
        }
    ...
    for (uint32_t v_addr = 0; size_t(v_addr) < mFileEntryTable.size();)
    {
        ...
        uint32_t entry_total_size =
            sizeof(pie::hac::sRomfsFileEntry) + align<uint32_t>(getFileEntry(v_addr)->name_size.unwrap(), 4);
