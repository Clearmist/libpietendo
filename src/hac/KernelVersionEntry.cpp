...
pie::hac::KernelVersionEntry::KernelVersionEntry() : mCap(kCapId), mVerMajor(0), mVerMinor(0) {}

pie::hac::KernelVersionEntry::KernelVersionEntry(const KernelCapabilityEntry &kernel_cap)
    : mCap(kCapId), mVerMajor(0), mVerMinor(0)
{
...
{
    return (mVerMajor == other.mVerMajor) && (mVerMinor == other.mVerMinor);
}
