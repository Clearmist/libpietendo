...
pie::hac::ThreadInfoEntry::ThreadInfoEntry()
    : mCap(kCapId), mMinPriority(kDefaultPriority), mMaxPriority(kDefaultPriority), mMinCpuId(kDefaultCpuId),
...
pie::hac::ThreadInfoEntry::ThreadInfoEntry(
    uint8_t min_priority, uint8_t max_priority, uint8_t min_core_number, uint8_t max_core_number)
    : mCap(kCapId), mMinPriority(kDefaultPriority), mMaxPriority(kDefaultPriority), mMinCpuId(kDefaultCpuId),
      mMaxCpuId(kDefaultCpuId)
{
...
{
    return (mMinPriority == other.mMinPriority) && (mMaxPriority == other.mMaxPriority) &&
           (mMinCpuId == other.mMinCpuId) && (mMaxCpuId == other.mMaxCpuId);
}
