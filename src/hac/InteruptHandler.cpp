...
pie::hac::InteruptHandler::InteruptHandler() : mIsSet(false), mInterupts() {}
...
{
    return (mIsSet == other.mIsSet) && (mInterupts == other.mInterupts);
}
...
void pie::hac::InteruptHandler::exportKernelCapabilityList(std::vector<KernelCapabilityEntry> &caps) const
{
    ...
    for (; i < mInterupts.size(); i += 2)
    {
        ...
        {
            caps.push_back(
                InteruptEntry(InteruptEntry::kInteruptMax, InteruptEntry::kInteruptMax).getKernelCapability());
        }
