...
pie::hac::BKTREncryptedStream::BKTREncryptedStream()
    : mLength(0), mCurrentPos(0), mIndirectEntries(), mBaseReader(nullptr), mPatchReader(nullptr)
{
}
...
pie::hac::BKTREncryptedStream::BKTREncryptedStream(
    ...
    const pie::hac::sContentArchiveFsHeaderPatchInfo &patch_info,
    const std::shared_ptr<tc::io::IStream> &base)
    : BKTREncryptedStream()
{
    ...
    if (sections_reader.length() <
        int64_t(patch_info.indirect_bucket.offset.unwrap() + patch_info.indirect_bucket.size.unwrap()))
    {
    ...
    const pie::hac::sIndirectStorageBlock *indirect_block =
        (const pie::hac::sIndirectStorageBlock *)indirect_block_raw.data();

    // Prepare base reader (from base NCA) and patch reader (from update NCA)
    mBaseReader = base;
    mPatchReader = std::make_shared<pie::hac::BKTRSubsectionEncryptedStream>(
        pie::hac::BKTRSubsectionEncryptedStream(stream, key, counter, patch_info.aes_ctr_ex_bucket));
        ...
        for (size_t j = bucket.header.entry_count; j > 0; --j)
        {
            ...
            default:
                throw tc::NotSupportedException(
                    kClassName, fmt::format("StorageSource({:d}) not supported", entry.storage_index.unwrap()));
                break;
            }
...
size_t pie::hac::BKTREncryptedStream::read(byte_t *ptr, size_t count)
{
    ...
    // map must to contain an entry for offset 0, but never retrieve by upper bound (because the key returned must be
    // greater) and in case ask for a offset greater (or equal) to last entry its return "end", so its safe to get
    // previous entry
